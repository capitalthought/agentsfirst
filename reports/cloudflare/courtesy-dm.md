...
We just shipped an Agent Readiness Report on Cloudflare against the Agents First rubric (the one your April Agent Readiness Score post is the canonical source for). www.cloudflare.com scored 85/100, Level 3 — top-tier in the series. Real respect: you wrote the rubric, scored yourself, and shipped the artifacts on the marketing root. That sequence is the credibility move.

The variance is the story though — developers.cloudflare.com scored 35/100 (Level 2), and blog.cloudflare.com scored 0/100 (Level 0, no agent access at all). Same engineering culture, demonstrably the same playbook one subdomain over. Closing the dev-portal gap is a flat-file edit (copy /AGENTS.md, /agents.json, named-bot robots.txt over) — ~30 min for +50 points on the surface that arguably matters more. The blog is the bigger surprise — whatever's blocking agents there is presumably an artifact of a different team's robots.txt defaults, not intent.
...
| John Graham-Cumming | CTO, Cloudflare | Long-running tech blogger; would respect the rubric and the credibility frame. Also the natural owner-adjacent person for the blog.cloudflare.com 0/100 finding. | LinkedIn DM — verify handle first |
...
- "The blog 0/100 is a robots.txt default we didn't realize was there" → great, even simpler fix; same follow-up pattern, and the "we didn't realize" framing is itself a useful story about how blocking-by-default propagates.
...
- **Lead with the score, not the critique.** 85/100 is real respect; the dev-portal and blog observations are constructive, not snarky.
...
- **The blog 0/100 is the spicy finding — handle it gently.** It's a worse score than most companies in the series get on any surface, and it's on Cloudflare, the company that wrote the rubric. Frame as "presumably an artifact, not intent" in the DM; let the public post note the irony more directly if it needs to.
