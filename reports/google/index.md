...
Methodology note: this score uses the v0.1.2 rubric, which credits the `Content-Signal` directive in `robots.txt` (Cloudflare's convention) and per-named-bot blocks for `GPTBot`, `ClaudeBot`, `anthropic-ai`, `Google-Extended`, `PerplexityBot`, `CCBot`. None of the four Google surfaces declared any of them. Source: <https://github.com/capitalthought/agentsfirst/blob/main/tools/agentsfirst-mcp/src/score.ts>.
