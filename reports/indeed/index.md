...
Methodology note: this score uses the v0.1.2 rubric. We verified that Indeed's `/llms.txt` does in fact exist and serve real content to browser-spoofing user-agents (`curl -A "Mozilla/5.0"` returns the 737-line file). The scorer fetches with a default user-agent and is blocked at the WAF; the rubric scores what an agent can actually reach, which is the right behavior. Source: <https://github.com/capitalthought/agentsfirst/blob/main/tools/agentsfirst-mcp/src/score.ts>.
