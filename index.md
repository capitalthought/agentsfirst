...
| 9 | **Inspectable State**{:#inspectable-state} | Every agent server exposes its own operational state — queue depth, throughput, recent activity, trends, health — via a typed agent tool, not just a human dashboard. Where Prep Gates answers "is the system ready?", Inspectable State answers "what is the state of the work?". |
...
**Gate it in CI.** You can fail builds when your agent-readiness regresses, the same way you'd fail on a broken test. [`lingzhong/check-agent-readiness`](https://github.com/marketplace/actions/check-agent-readiness) (GitHub Action, v0.1.1, 2026-05-14) runs a configurable agent-readiness scanner — Cloudflare's [isitagentready.com](https://isitagentready.com) by default — against your site on every deploy and fails CI if the level drops below a threshold. That's the discovery-lane gate. For the principle-lane equivalent, point the same pattern at `agentsfirst.dev/mcp` — the hosted scorer responds to `score_website` and returns a `total` you can threshold against. Two gates, two lenses, same idea: catch regressions before agents do.
